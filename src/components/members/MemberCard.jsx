...
const MemberCard = ({ member, index = 0, spotlight = false }) => {
  const hasSocials = Boolean(member.linkedin || member.github || member.portfolio);
        ...
        <div className={spotlight ? "md:w-2/3 space-y-5 mt-6 md:mt-0" : ""}>
          <div className={spotlight ? "space-y-3" : "flex items-start gap-4"}>
            {!spotlight ? (
              <div className="space-y-2">
                <span className={`badge ${roleStyles[member.role] || "badge-primary"} badge-outline`}>
                  {member.role}
                </span>
              ...
              <>
                <span className={`badge ${roleStyles[member.role] || "badge-primary"} badge-outline badge-lg`}>
                  {member.role}
                </span>
          ...
          <div className="flex items-center justify-between gap-4 pt-2">
            ...
            <span className="text-xs uppercase tracking-[0.2em] text-accent/70">
              {hasSocials ? "Connect" : "EWUCSC Panel"}
            </span>
          </div>
        </div>
      </div>
