...
const UpcomingCtfs = ({ limit = 6, compact = false }) => {
    ...
    return (
      <div className="grid gap-4 md:grid-cols-2">
        {Array.from({ length: compact ? 2 : 6 }).map((_, index) => (
          <div
            key={index}
            className="h-36 animate-pulse rounded-2xl border border-white/5 bg-base-200/50"
          />
        ))}
      </div>
    );
  ...
  // When Show More is clicked, show all events.
      ...
      {events.length > limit && (
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="font-mono text-sm font-bold uppercase tracking-[0.14em] text-secondary transition hover:text-secondary/70"
          >
